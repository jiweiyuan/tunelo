...
pub async fn run(domain: String, tunnel_addr: String, http_addr: String) -> Result<()> {
    info!(domain = %domain, tunnel = %tunnel_addr, http = %http_addr, "starting relay");
    ...
    let tunnel_task = tokio::spawn(async move {
        tunnel::run_tunnel_listener(t_addr, quic_config, r1, d).await
    });
